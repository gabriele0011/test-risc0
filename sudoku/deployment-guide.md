...
4. Deploy your contract by running:
    ...
    ```bash
    export SUDOKU_ADDRESS=#COPY EVEN NUMBER ADDRESS FROM DEPLOY LOGS
    ```

    > You can also use the following command to set the contract address if you have [`jq`][jq] installed:
    >
    > ```bash
    > export SUDOKU_ADDRESS=$(jq -re '.transactions[] | select(.contractName == "EvenNumber") | .contractAddress' ./broadcast/Deploy.s.sol/31337/run-latest.json)
    > ```
...
1. Query the state:
    ...
    ```bash
    cast call --rpc-url http://localhost:8545 ${SUDOKU_ADDRESS:?} 'get()(bytes memory)'
    ```
...
3. Deploy your contract by running:
    ...
    >
    > ```bash
    > export SUDOKU_ADDRESS=$(jq -re '.transactions[] | select(.contractName == "EvenNumber") | .contractAddress' ./broadcast/Deploy.s.sol/11155111/run-latest.json)
    > ```
...
3. Deploy your contract by running:
    ...
    >
    > ```bash
    > export SUDOKU_ADDRESS=$(jq -re '.transactions[] | select(.contractName == "EvenNumber") | .contractAddress' ./broadcast/Deploy.s.sol/1/run-latest.json)
    > ```
...
1. Query the state:
    ...
    ```bash
    cast call --rpc-url https://eth-mainnet.g.alchemy.com/v2/${ALCHEMY_API_KEY:?} ${SUDOKU_ADDRESS:?} 'get()(uint256)'
    ```
