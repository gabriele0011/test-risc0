...
4. Deploy your contract by running:
    ...
    ```bash
    export INSERTION_SORT_ADDRESS=#COPY EVEN NUMBER ADDRESS FROM DEPLOY LOGS
    ```
...
2. Publish a new state
    ...
    cargo run --bin publisher -- \
        ...
        --contract=${INSERTION_SORT_ADDRESS:?} \
...
3. Deploy your contract by running:
    ...
    >
    > ```bash
    > export INSERTION_SORT_ADDRESS=$(jq -re '.transactions[] | select(.contractName == "InsertionSort") | .contractAddress' ./broadcast/Deploy.s.sol/11155111/run-latest.json)
    > ```
...
1. Query the state:
    ...
    ```bash
    cast call --rpc-url https://eth-sepolia.g.alchemy.com/v2/${ALCHEMY_API_KEY:?} ${INSERTION_SORT_ADDRESS:?} 'get()(int256[])'
    ```
