...
4. Deploy your contract by running:
    ...
    Deployed MergeSort to 0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512
    ...
    ```

    Save the `MergeSort` contract address to an env variable:

    ```bash
    export MERGE_SORT_ADDRESS=#COPY EVEN NUMBER ADDRESS FROM DEPLOY LOGS
    ```

    > You can also use the following command to set the contract address if you have [`jq`][jq] installed:
    >
    > ```bash
    > export MERGE_SORT_ADDRESS=$(jq -re '.transactions[] | select(.contractName == "MergeSort") | .contractAddress' ./broadcast/Deploy.s.sol/31337/run-latest.json)
    > ```
...
3. Deploy your contract by running:
    ...
    >
    > ```bash
    > export MERGE_SORT_ADDRESS=$(jq -re '.transactions[] | select(.contractName == "MergeSort") | .contractAddress' ./broadcast/Deploy.s.sol/11155111/run-latest.json)
    > ```
...
1. Query the state:
    ...
    ```bash
    cast call --rpc-url https://eth-sepolia.g.alchemy.com/v2/${ALCHEMY_API_KEY:?} ${MERGE_SORT_ADDRESS:?} 'get()(int256[])'
    ```
