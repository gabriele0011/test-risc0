...
# 5. Estrai indirizzo CountingSort dal log di deploy
export COUNTING_SORT_ADDRESS=$(jq -re '.transactions[] | select(.contractName == "CountingSort") | .contractAddress' ./broadcast/Deploy.s.sol/31337/run-latest.json)

# Aggiorna il file delle variabili d'ambiente
echo "export ETH_WALLET_PRIVATE_KEY=0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80" > .env_vars
echo "export COUNTING_SORT_ADDRESS=$COUNTING_SORT_ADDRESS" >> .env_vars

# 6. Query stato iniziale
cast call --rpc-url http://localhost:8545 $COUNTING_SORT_ADDRESS 'get()(int32[])'
...
echo "    --contract=\"$COUNTING_SORT_ADDRESS\" \\"
