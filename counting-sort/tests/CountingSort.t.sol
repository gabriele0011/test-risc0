/*
 *    The following test instantiates the IRiscZeroVerifier and countingSort smart contracts and generates a proof that will
 *    then be verified in the countingSort smart contract. All the program logic, including proof generation via the prove
...
import {CountingSort} from "../contracts/CountingSort.sol";
import {Elf} from "./Elf.sol"; // auto-generated contract after running `cargo build`.

contract CountingSortTest is RiscZeroCheats, Test {
    
    CountingSort public countingSort;

    // Helper to assert equality between two int32[] arrays
    function assertEqInt32Arrays(int32[] memory a, int32[] memory b) internal {
    ...
    function setUp() public {
        ...
        countingSort = new CountingSort(verifier);
    }
    ...
    function test_SortUnsortedArray() public {
        ...
        // Checik that the journal output is the correctly sorted array.        // Check that the journal output is the correctly sorted array.
        assertEqInt32Arrays(provenSortedArray, expectedSortedArray);

        // Set the state on the contract.
        countingSort.set(provenSortedArray, seal);

        // Verify that the contract now holds the sorted array. Verify that the contract now holds the sorted array.
        assertEqInt32Arrays(countingSort.get(), expectedSortedArray);
    }
}
