...
the CountingSort smart contract and is publicly verifiable.
...
use clap::Parser;
use methods::COUNTING_SORT_ELF;
use risc0_ethereum_contracts::encode_seal;
use risc0_zkvm::{default_prover, ExecutorEnv, ProverOpts, VerifierContext};
use url::Url;

// `ICountingSort` interface automatically generated via the alloy `sol!` macro.
alloy::sol!(
    #[sol(rpc, all_derives)]
    "../contracts/ICountingSort.sol"
);
...
fn main() -> Result<()> {
    ...
    let contract = ICountingSort::new(args.contract, provider);
    ...
    // effectively calling the set function of the CountingSort contract with the verified number and proof.
